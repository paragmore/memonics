...
def create_follower(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if not instance.pk == 1:
            Follower.objects.create(follower=instance, being_followed=Admin)
            Follower.objects.create(follower=Admin, being_followed=instance)
...
def create_objects(sender, instance, created, **kwargs):
    Admin = User.objects.get(id=1)
    if created:
        Message.objects.create(sender=Admin, receiver=instance, content='This is your inbox! Click me to view our conversation.')
    ...
    #     Comment.objects.create(post=post, author=Admin, content='Here is your first comment!')
